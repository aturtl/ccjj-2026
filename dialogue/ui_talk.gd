...
func play(s: String, starting_visible_characters: int = 0):
	var box = add_box_instance(box_template.duplicate())
	var label = box.label
...
func add_box_instance(box: BoxTemplate):
	add_child(box)
